...
if ( AcpService::deCodeFileContent( $result_arr, SDK_FILE_DOWN_PATH ) == false) {
...
function analyze_file($fileName){
	...
	if ($zip->open ( SDK_FILE_DOWN_PATH . "/" . $fileName ) === TRUE) {
		...
		foreach (scandir(SDK_FILE_DOWN_PATH) as $fileName) {
			$list = null;
			if (substr($fileName, 0, 3) == "INN" && substr($fileName, 11, 3) == "ZM_")
				$list = parse_file_zm ( SDK_FILE_DOWN_PATH."/".$fileName ); //处理流水文件
			else if (substr($fileName, 0, 3) == "INN" && substr($fileName, 11, 4) == "ZME_")
				$list = parse_file_zme ( SDK_FILE_DOWN_PATH."/".$fileName ); //处理差错流水文件
